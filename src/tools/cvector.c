...
do {\
    if ((cvector_begin(cv) > iter) || (cvector_end(cv) <= iter)) {\
        fprintf(stderr, "var(" #iter ") warng out of range, at file:%s func:%s line:%d!!/n", file, func, line);\
        return CVEFAILED;\
}\
} while (0)

extern cvector   cvector_create(const size_t size);
extern void      cvector_destroy(const cvector cv);
extern size_t    cvector_length(const cvector cv);
extern int       cvector_pushback(const cvector cv, void *memb);
extern int       cvector_popback(const cvector cv, void *memb);
extern size_t    cvector_iter_at(const cvector cv, citerator iter);
extern int       cvector_iter_val(const cvector cv, citerator iter, void *memb);
extern citerator cvector_begin(const cvector cv);
extern citerator cvector_end(const cvector cv);
extern citerator cvector_next(const cvector cv, citerator iter);
extern int       cvector_val_at(const cvector cv, size_t index, void *memb);
extern int       cvector_insert(const cvector cv, citerator iter, void *memb);
extern int       cvector_insert_at(const cvector cv, size_t index, void *memb );
extern int       cvector_rm(const cvector cv, citerator iter);
extern int       cvector_rm_at(const cvector cv, size_t index);

/* for test */
extern void cv_info(const cvector cv);
extern void cv_print(const cvector cv);
